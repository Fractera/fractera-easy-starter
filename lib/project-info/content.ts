...
- **Страница** — URL, где вы проверяете изменение (последняя колонка), рядом с меткой **Created** (дата создания) и привычными ячейками коммита / статуса / длительности / ветки / автора. Каждая колонка сортируется, а развёртывания можно разделять по **проектам** (списком проектов вы управляете) и фильтровать таблицу по одному или нескольким из них.

**Почему это петля.** Hermes принимает решение, делегирует разработку одному или нескольким агентам, деплоит, затем записывает результат и отдаёт вам ссылку на страницу для проверки. Вы ставите оценку. Со временем таблица становится записью обратной связи по AI-разработке — какой агент и какая модель дают лучший результат для вашего проекта и по какой цене в токенах — то, чего журнал деплоев в одиночку не покажет. Это видимое сердце «продуктовой петли» Fractera: собрать, развернуть, проверить, оценить, улучшить. Весь цикл AI-разработки наглядно показан на странице [цикл разработки Fractera](/ai-development-loop).`,
  },
  {
    id: 'memory-flow',
...
- **Partner page** — a personal mirror of the Fractera landing at \`https://www.fractera.ai/partner/<lang>/<slug>\`, trimmed to just the order section (enter server details, pick a provider). It also satisfies what hosting providers require for affiliate approval: a real URL where the affiliate link lives.
...
- **Партнёрская страница** — персональное зеркало лендинга Fractera по адресу \`https://www.fractera.ai/partner/<lang>/<slug>\`, сокращённое до одной секции оформления заказа (ввод данных сервера, выбор провайдера). Она же закрывает требование хостингов для одобрения affiliate: реальный URL, где живёт партнёрская ссылка.
...
- **Why ours stands up to it.** The packages we install are maximally transparent — all from open source, so they can be inspected by any security team or accredited auditor without a shred of doubt. Source of truth: https://github.com/Fractera/Agentic-Engineering-Infrastructure.
...
- **Почему наш код это выдерживает.** Устанавливаемые пакеты максимально прозрачны — все из открытого исходного кода, поэтому их могут проверить любые службы безопасности или аккредитованные аудиторы без тени сомнения. Источник истины: https://github.com/Fractera/Agentic-Engineering-Infrastructure.
