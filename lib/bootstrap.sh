...
# 🛑 BUN — РАНТАЙМ ОПРАШИВАТЕЛЯ TELEGRAM, И БЕЗ НЕГО КАНАЛ МЁРТВ МОЛЧА.
# Официальный плагин `telegram@claude-plugins-official` запускает свой MCP-сервер
# как `bun server.ts` (`server.ts:1` → `#!/usr/bin/env bun`), и его README
# называет Bun обязательным. Ни один шаг установки его не ставил.
#
# ✗ ОПЛАЧЕНО ПЕРВЫМ РАЗВЁРТЫВАНИЕМ НА ЧИСТУЮ МАШИНУ 2026-09-05, ЧАСОМ ОЖИДАНИЯ
# ВЛАДЕЛЬЦА. Отказ выглядел как успех по всем признакам, которые мы проверяли:
# десять процессов `online`, вход в Claude выполнен, плагин установлен и включён,
# токен верной формы на месте, порт 3600 отвечает. Опрашиватель — ноль, бот
# молчал, и в журнале установки об этом не было ни строки.
#
# 🔒 ССЫЛКА В `/usr/local/bin` ОБЯЗАТЕЛЬНА, А НЕ ДЛЯ КРАСОТЫ. Установщик Bun
# кладёт бинарь в `~/.bun/bin` и правит профиль оболочки — а агент работает под
# `pm2`, который профиль не читает. Без ссылки `bun` есть на диске и не найден
# процессом, то есть тот же молчаливый отказ, только на шаг позже.
#
# 🔒 ШАГ МЯГКИЙ, КАК И ВЕСЬ БЛОК: сервер без Telegram-канала — это работающий
# сервер и строка в журнале, а не упавшая установка.
soft_step "chat_bun" "Bun (Telegram poller runtime)" \
  "command -v bun >/dev/null 2>&1 || (curl -fsSL https://bun.sh/install | bash); [ -x \$HOME/.bun/bin/bun ] && ln -sf \$HOME/.bun/bin/bun /usr/local/bin/bun; command -v bun >/dev/null 2>&1"
...
# их ДЕСЯТЬ: postgres · db · pnpm · clone · env · deps · build · start · channels_link · agent_cli.
# 🔒 Девятый добавлен шагом 110 (2026-09-04), десятый — шагом 114 (2026-09-04), и число
# исправлено ЭТОЙ ЖЕ правкой — закон, оплаченный в корпусе пять раз: рукописное число не
# двигается само никогда.
#
# ✗ И ТУТ ЖЕ, ШАГОМ 114, НАЙДЕНО ПРОДОЛЖЕНИЕ ТОГО ЖЕ ОБВАЛА 2026-09-03: правка оставила
# ДВЕ СТРОКИ-СИРОТЫ — здесь и у `start_chat`. Обе выглядели как команды шага, но стояли
# сами по себе, без `soft_step` перед ними. `set -e` в этом файле НЕТ (см. строку 144), и
# такая строка даёт `command not found`, код 127 — и установка молча идёт дальше.
# 🛑 ЦЕНА СИРОТЫ ЗДЕСЬ БЫЛА НЕ НУЛЕВОЙ ПО ЗАМЫСЛУ: в ней жила ЕДИНСТВЕННАЯ строка,
# вычищающая токен из `.git/config` чата. Она не исполнялась НИ РАЗУ.
...
# ── СЛУЖБА ПАМЯТИ (шаг 175-1, 2026-09-09) ──────────────────────────────────────
#
# 🔒 ПАМЯТЬ — ОТДЕЛЬНАЯ СЛУЖБА, А НЕ ЧАСТЬ БОТА, И ЭТО РЕШЕНИЕ ВЛАДЕЛЬЦА.
# Дословно: «память должна быть отделена от агента гораздо сильнее, отделена на
# физическом уровне… это должно выглядеть, как будто мы обращаемся в облако
# Google — просто отправляем запрос, получаем результат».
# ✗ ЦЕНА ПРЕЖНЕГО УСТРОЙСТВА ИЗМЕРЕНА 2026-09-09: память жила внутри приложения
# бота (5 997 строк), сервер инструментов агента импортировал её исходники ПО
...
#
# 🛑 ШАГИ МЯГКИЕ ПО ТОЙ ЖЕ ПРИЧИНЕ, ЧТО У СОСЕДЕЙ: сервер без службы памяти —
# это работающий сервер и строка в журнале, а не упавшая установка.
...
# чата и появится сама; сам `claude` — чужой пакет, и без этой строки следующий сервер
# родится с вкладкой, у которой режимы входа ничего не запускают.
# ✗ ИМЕННО ЭТОТ КЛАСС ОПЛАЧЕН ДВАЖДЫ ЗА НЕДЕЛЮ: чата не было в bootstrap (шаг 96), связки
# каналов не было (шаг 110). Оба раза «сделанное руками жило на одной машине».
#
# 🔒 ШАГ МЯГКИЙ, И ЭТО ВЕРНО ПО СУЩЕСТВУ: сервер без агента-программиста — это работающий
