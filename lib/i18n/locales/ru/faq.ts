...
export const faqItems: FaqItem[] = [
  ...
  }, */
  {
    ...
    a: [
      'Да — партнёрская программа Fractera. Модель проста: рефералы на Contabo.',
      'Подайте заявку на партнёрство в Contabo. Это доверенный, проверенный временем хостинг — учтите, что Contabo не выдаёт партнёрскую ссылку мгновенно: они проверяют страницу, на которой будет размещена ссылка, и запрашивают базовые юридические данные о вашей компании. После одобрения передайте ссылку нам через страницу партнёров, и мы разместим её в блоке рекомендованных серверов на лендинге. Каждый приведённый клиент даёт 20 € за нашу рекомендованную конфигурацию или 30 €+ за более мощные серверы — подробности выплат на сайте партнёрской программы Contabo.',
    ],
    trail: [
      'Чтобы присоединиться, см. страницу партнёров — там описан полный процесс.',
  ...
  },
  {
    ...
    a: [
      'Ваш доход от сотрудничества проще и ближе, чем кажется. Короткое согласование с Contabo — и пассивные выплаты, пока ваш контент привлекает зрителей.',
    ],
    steps: [
      'Подайте заявку аффилиата на странице партнёрской программы Contabo. Они спросят URL, где будет жить партнёрская ссылка, и базовые юридические данные. Проверка обычно занимает несколько рабочих дней.',
      'После одобрения Contabo скопируйте свою партнёрскую ссылку.',
      'Откройте вкладку «Партнёрская программа» на странице партнёров, создайте партнёрский кабинет и загрузите ссылку Contabo.',
      'Вы получите персональное зеркало лендинга Fractera, где ваша ссылка Contabo заменит нашу.',
      'Рекомендуйте Fractera в блоге, YouTube-канале, постах в соцсетях — везде, где собираются разработчики.',
      'Любой посетитель по вашей ссылке, который купит сервер Contabo, даёт выплату: 20 € за рекомендованную конфигурацию, 30 €+ за более мощные серверы — Contabo платит напрямую вам, Fractera в транзакции не участвует.',
    ],
    trail: [
      'Математика простая: одно одобрение в Contabo — и пассивные выплаты, пока статья, пост или видео приносят трафик. Contabo жёстко конкурирует за новых клиентов — и вознаграждает вас за каждого, кого вы приводите в их экосистему.',
    ],
  },
