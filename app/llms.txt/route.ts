...
- [News](https://www.fractera.ai/en/news): chronological, AI-searchable project updates — new framework starters, AI-workspace features, agent skills and MCP tools — each embedded into the on-server LightRAG vector memory the moment it ships. First story — [AI Draft Settings](https://www.fractera.ai/en/news/ai-draft-settings-evolutionary-pipeline): the page (included with every Next.js-based starter) where you or any of the six AI agents draft a new skill, instruction or MCP connector and stage it safely before it reaches the real files. Drafts become "Next Step" entries on the Development Steps page; turning a draft into code is never automatic on creation (to protect the agent's context window and token budget). It is the first link in a seven-stage pipeline for evolving a workspace, and the same depth is being brought to other frameworks via dedicated AI-workspace transports.
