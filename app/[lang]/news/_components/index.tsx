...
// Standard router shape: page.tsx is thin and re-exports this. The post list is
// auto-discovered: POSTS comes from _list.generated.ts (built by lib/parser-fs
// from the co-located post folders); newsList aggregates + localizes it.
...
export default async function NewsIndex({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params
  const ui = getNewsUi(lang)
  const news = newsList(POSTS, lang) // localized title + summary, from each post's _data
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: ui.breadcrumbNews, item: `${BRAND.siteUrl}/${lang}/news` },
    ],
  }
