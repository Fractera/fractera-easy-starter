...
export const ru: NewsArticleOverride = {
  ...
  blocks: [
    ...
    {
      kind: 'h2',
      text: 'Теперь у автоматизации есть объявленный вход и выход',
    },
    {
      kind: 'p',
      text: 'Любая автоматизация — это преобразование: она читает из **входа** и пишет в **выход**. Раньше эта граница была неявной — и ИИ мог «молча» скопировать чужую форму: наш собственный сид «курсы для ребёнка» когда-то вышел как «вход Telegram → выход Telegram», хотя его настоящий выход — автономная интерактивная страница-урок. Теперь граница — это **объявленный контракт портов** на графе: порты берутся из замкнутого словаря (мессенджер, страница, хранилище, расписание, событие, ручной запуск, внешний API), показываются в шапке проекта и проверяются гейтом — выход, который не производит ни один узел, подсвечивается. И **выход — это список**: один новостной агент может публиковать пост и на сайт, и в Medium по API одновременно.',
    },
    {
      kind: 'h2',
      text: 'Автоматизация, которая создаёт живые страницы',
    },
    {
      kind: 'p',
      text: 'Выход бывает не только сообщением в чат. Автоматизация может материализовать **автономную страницу** — курс, анкету, доску Kanban — через шлюз между системой автоматизаций и системой генерации сайта. Такая страница живёт сама после прогона и даже собирает данные обратно (ученик заполняет квиз — и это снова вход). Шлюз — типизированная граница с двумя плоскостями: одной автоматизация страницу **создаёт и модерирует**, другой — **взаимодействует** с ней в рантайме (добавить карточку, передвинуть карточку). Всё объявляется манифестом, а не пишется руками под каждую доску.',
    },
    {
      kind: 'callout',
      title: 'Спроектировал дорого один раз — исполняешь дёшево вечно',
      text: 'Это экономическое ядро подхода: автоматизацию вы проектируете один раз сильной (дорогой) моделью, а дальше она работает автономно на самой дешёвой модели для каждого повторяющегося цикла. Пример: человек ищет работу и автоматизирует отклики в LinkedIn и поиск контактов на сайтах работодателей — несколько минут проектирования дорогой моделью, а потом дешёвая модель гоняет это каждый день. Альтернатива — дёргать дорогую модель на каждый запрос напрямую — в сумме съедает весь бюджет безработного человека.',
    },
    {
      kind: 'h2',
