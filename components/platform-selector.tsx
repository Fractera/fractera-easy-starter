...
// page hero at the top of /deployments/mcp) while this section keeps only the
// connector details (agents + server URL). Defaults to true so other usages are
// unaffected.
export function PlatformSelector({ showSlider = true }: { showSlider?: boolean } = {}) {
  ...
  return (
    <div id="mcp-section" className="flex flex-col gap-6 w-full scroll-mt-16">
      ...
      {/* Step-by-step slider — rendered here by default, but suppressed when the
          carousel is shown elsewhere (e.g. as the page hero on /deployments/mcp). */}
      {showSlider && <div className="mt-4"><McpStepSlider /></div>}
    </div>
  )
}
