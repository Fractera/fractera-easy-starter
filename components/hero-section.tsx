...
import { Layers, Lightbulb, Code2, Rocket, TrendingUp, Brain, Target, Smartphone, AlertCircle, Mic, ShieldCheck, Database, GitBranch, Zap, ShoppingBag, Globe, Crosshair, Bot, RefreshCcw } from 'lucide-react'
...
const SHOWCASE_ICONS = [Layers, RefreshCcw, ShoppingBag]
...
const SHOWCASE_ACCENTS = ['violet', 'cyan', 'amber'] as const
const SHOWCASE_PLACEHOLDER_LABELS = ['Build Products diagram', 'Hermes Loop diagram', 'Skills Marketplace diagram']
...
function LoopShowcase() {
  ...
  const Icon = SHOWCASE_ICONS[activeIdx]
  const gradient = SHOWCASE_GRADIENTS[activeIdx]
  const accent = SHOWCASE_ACCENTS[activeIdx]
  const placeholderLabel = SHOWCASE_PLACEHOLDER_LABELS[activeIdx]
    ...
    <section className="w-full max-w-5xl mx-auto px-4 flex flex-col items-center gap-8">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-3">
        {slides.map((slide, i) => {
          ...
          const btnAccent = SHOWCASE_ACCENTS[i]
          const activeClass =
      ...
      <div className="max-w-4xl mx-auto px-6 flex flex-col items-center justify-between gap-8 md:flex-row md:gap-16">
        <div className="flex flex-col items-center text-center md:items-start md:text-left">
          ...
          <div className="flex flex-wrap gap-4 mt-8">
            <a
              href="#pricing"
              className="inline-flex items-center gap-2 justify-center rounded-full bg-violet-600 hover:bg-violet-500 text-white font-bold px-8 py-3 transition-colors shadow-lg shadow-violet-500/30"
            >
              {content.deployButton}
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </a>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 justify-center rounded-full border border-violet-500/50 bg-violet-500/[0.06] hover:bg-violet-500/20 text-white font-semibold px-6 py-3 transition-colors"
            >
...
const FAQ_ITEMS: FaqItem[] = [
  ...
  },
  {
    ...
    a: [
      'For full AI-coding workloads, the recommended minimum is 4 cores and 6 GB RAM. Storage depends on your project — 75 GB is a solid baseline, though go larger if you plan to host video or media files.',
